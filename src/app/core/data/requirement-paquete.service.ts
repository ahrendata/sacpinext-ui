...
const paqueterequirementIdName = 'PaqueteRequirement';
const paqueterequirementsPath = 'PaqueteRequirement';

@Injectable()
export class RequirementPaqueteService {
    ...
    findById(id: number, queryParams?: URLSearchParams): Observable<ContainerTypeCode> {
        const restangular = this.restangular.one(paqueterequirementsPath, id);
        return restangular
            .get(queryParams)
            .map(response => {
    ...
    getAll(queryParams?: URLSearchParams): Observable<ContainerTypeCode[]> {
        const restangular = this.restangular.all(paqueterequirementsPath);
        return restangular
            .get(queryParams)
            .map(response => {
                ...
                json.forEach(element => {                  
                    const typerequirement = new ContainerTypeCode(restangular.one('', element[paqueterequirementIdName]));
